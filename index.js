...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.dc8pd.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`;
console.log(uri)
...
async function run() {
  try {
    ...
    app.get('/my-visas/:id', async(req,res)=>{
    ...
    app.post('/my-visas', async(req,  res)=>{
      ...
      console.log('new visa ', visas);
      const result = await visasCollection.insertOne(visas);
      res.send(result);
    })
...
app.get('/', (req,res)=>{
    res.send('Simple Visa Website is Running')
})
